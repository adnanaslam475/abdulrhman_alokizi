...
import cx from "./ManageDevices.module.scss";
...
import { userDeviceFilterValue, userDeviceList, userDeviceListItems } from "../../redux_toolkit/reducer/manageDeviceApiReducer";
      ...
      >
        <div className={`${table.mainTitleRow}`}>
          <ul className={`${table.filterList}`}>
            ...
            <li>
              <button className={`btn ${table.filterBtn}`}>KDS</button>{" "}
            </li>
            <li>
              <button className={`btn ${table.filterBtn}`}>Notifer</button>{" "}
            </li>
            <li>
              <button className={`btn ${table.filterBtn}`}>Display</button>{" "}
            </li>
            <li>
              <button className={`btn ${table.filterBtn}`}>Sub Cashier </button>{" "}
            </li>
          </ul>
          <ul className={`${table.rightActionIcons}`}>
            <li>
              <button
                ...
                onClick={() => {
                  handleShow("devices filter", true);
                  console.log("check");
                }}
              >
...
function EnhancedTable() {
  ...
  const userDeviceListCount = useSelector((state:any)=>state.manageDeviceApiReducer.userDeviceListCount)
  ...
  useEffect(() => {
    if (rows?.length === 0) {
      userDeviceFilterValue.type = "all";
      dispatch(userDeviceList(userDeviceFilterValue));
    }
  }, []);

  useEffect(() => {
    setloaderStatus(<ReactLoading type="cylon" color="#5498fd" />)
    const rowsData = userDeviceListItems?.map((item: any) => {
      return {
        branch: item.branch,
        type: item.type,
        status: item.status,
    ...
    setRows(rowsData);
    if(userDeviceListItems?.length === 0 ){
                ...
                ?.map((row:any, index:number) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <>
                          <div >
                            <button className={`${cx.notUsedBtn}`}>   {row?.status}</button>
                          </div>
                        </>  
                      </TableCell>
                      <TableCell onClick={() => { redirectToAnotherPage(row?.id) }} align="left">
