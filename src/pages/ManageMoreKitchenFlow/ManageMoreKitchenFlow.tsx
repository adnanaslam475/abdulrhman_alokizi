...
import cx from "./ManageMoreKitchenFlow.module.scss";
...
import { kitchenFlowFilter, kitchenFlowList, kitchenFlowListItems, kitchenFlowPerId } from "../../redux_toolkit/reducer/manageMoreKitchenFlowApiReducer";
          ...
          <ul className={`${table.rightActionIcons}`}>
            <li>
              <button
                ...
                onClick={() => {
                  handleShow("kitchen filter", true);
                  console.log("check");
                }}
              >
...
function EnhancedTable() {
  ...
  function redirectToAnotherPage(id: string) {
    navigate(`${id}`);
    dispatch(kitchenFlowPerId({kitchenflow_id:`${id}`}))
  }
  useEffect(()=>{
    const restaurantId = JSON.parse(localStorage.getItem("___data")!)
    kitchenFlowFilter.restaurent_id=restaurantId?.restaurant_id
    kitchenFlowFilter.type = "all"
     dispatch(kitchenFlowList(kitchenFlowFilter))
   },[kitchenFlowState.addEditDeletekitchenFlowCount])
   ...
   useEffect(() => {
     ...
     const rowsData = kitchenFlowListItems?.map((item: any) => {
     ...
     setRows(rowsData);
     if(kitchenFlowListItems?.length === 0 ){
   ...
   }, [kitchenFlowState.kitchenFlowListCount]);
  return (
    <Box sx={{ width: "100%" }}>
...
export default function ManageMoreKitchenFlow() {
          ...
          <div className={`${st.pageTitleRow}`}>
            ...
            <div className={`${st.rowTitleRight}`}>
              ...
              <button className={`btn ${st.themeBtn}`}
                onClick={() => {
                  handleShow("Create Kitchen Flows", true);
                }}
              >
                Create Kitchen Flows
              </button>
            </div>
          </div>
        </div>
