...
import cx from "./ManageMoreDeliveryZones.module.scss";
...
import Modals from "../../components/Modals/ManageMoreDeliveryZoneM";
import { deliveryFilter, deliveryList, deliveryListItems, editDeliveryFunc } from "../../redux_toolkit/reducer/manageMoreDeliveryApiReducer";
          ...
          <ul className={`${table.rightActionIcons}`}>
            <li>
              <button
                ...
                onClick={() => {
                  handleShow("delivery zone filter", true);
                  console.log("check");
                }}
              >
...
// Modal show and hide handlers (Table cell) (Start)
  ...
  function openModalPoupOnTableCell(row:any) {
    ...
    cordinate:row?.coordinate
    }
    dispatch(editDeliveryFunc(data))
    handleShow("Edit delivery zone", true);
  }
  useEffect(() => {
  ...
  useEffect(()=>{
    deliveryFilter.type="all"
   dispatch(deliveryList(deliveryFilter))
  ...
  useEffect(() => {
    ...
    const rowsData = deliveryListItems?.map((item: any) => {
      console.log(item,"deliveryItems")
      return {
        id: item?.id,
        name:item?.area_name,
  ...
  }, [deliveryState.deliveryListCount]);
...
export default function ManageMoreDeliveryZones() {
          ...
          <div className={`${st.pageTitleRow}`}>
            ...
            <div className={`${st.rowTitleRight}`}>
              ...
              <button className={`btn ${st.themeBtn}`}
                onClick={() => {
                  handleShow("Create delivery zone", true);
                }}
              >
