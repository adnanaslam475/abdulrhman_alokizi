...
import cx from "./Menumodifiers.module.scss";
...
import { editModifierFunc, modifierFilter, modifierList, modifierListItems } from "../../redux_toolkit/reducer/menuModifiersApiReducer";
...
const rows = [
  {
    ...
    reference: "1",
    options: "Options (1)",
    linkedproducts: "Products (0)",
    created: "2020-03-24 08:30pm",
  },
  {
    id:"1",
    name: "صوصات",
          ...
          <ul className={`${table.rightActionIcons}`}>
            <li>
              <button
                ...
                onClick={() => {
                  handleShow("modifiers filter", true);
                  console.log("check");
                }}
              >
...
function EnhancedTable() {
  const modifierCount = useSelector((state:any)=>state.menuModifiersApiReducer)
...
// Modal show and hide handlers (Table cell) (Start)
  ...
  function openModalPoupOnTableCell(row:any) {
    let data = {
      id:row?.id,
      name: row?.name,
    ...
    dispatch(editModifierFunc(data))
    handleShow("edit modifier", true);
  }

  useEffect(()=>{
    modifierFilter.type = "all"
    dispatch(modifierList(modifierFilter))
  },[modifierCount.addModifierCount, modifierCount.deletModifierCount])
  ...
  useEffect(() => {
    ...
    const rowsData = modifierListItems?.map((item: any) => {
      console.log(item,"modifierItems")
      return {
        id: item?.id,
        name:item?.name,
        maximum_option: item?.maximum_option,
        minimum_option: item?.minimum_option,
  ...
  }, [modifierCount.modifierListCount]);
                ...
                ?.map((row:any, index:number) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell onClick={()=>openModalPoupOnTableCell(row)} align="left">{row?.reference}</TableCell>
                      <TableCell onClick={()=>openModalPoupOnTableCell(row)} align="left">{row?.options}</TableCell>
                      <TableCell onClick={()=>openModalPoupOnTableCell(row)} align="left">{row?.linkedproduct}</TableCell>
                      <TableCell onClick={()=>openModalPoupOnTableCell(row)} align="left">{row?.created}</TableCell>
                    </TableRow>
                  );
                })}
              {emptyRows > 0 && (
        ...
        <div className={`${st.pageTitle}`}>
          <div className={`${st.pageTitleRow}`}>
            ...
            <div className={`${st.rowTitleRight}`}>
              <NavLink to="/menu/modifiers/sort-modifier" className={` btn ${cx.moreOption}`}>
              ...
              <button className={`btn ${st.themeBtn}`}
                onClick={() => {
                  handleShow("create modifier", true);
                }}
              >
                Create Modifier
              </button>
