...
import { customerList, customers } from "../../redux_toolkit/reducer/menuCustomerApiReducer";
...
function EnhancedTable() {
  ...
  const customerlistcount = useSelector(
    (state: any) => state.menuCustomerApiReducer.customerListcount
  );
  ...
  useEffect(() => {
    ...
    const rowsData = customerList?.map((item: any) => {
      return {
        id: item?.id,
    ...
    if (customerList?.length === 0) {
      setloaderStatus(<h5 style={{ paddingTop: "15px" }}>Start managing your menu by creating Customers.</h5>)
    }
  }, [customerlistcount]);
  ...
  function redirectToAnotherPage(id: any) {
    navigate(`${id}`);
  }
  useEffect(() => {
    if (rows?.length === 0) {
                  ...
                  .map((row: any, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell onClick={() => { redirectToAnotherPage(row.id) }} align="left">{row.phone}</TableCell>
                        <TableCell onClick={() => { redirectToAnotherPage(row.id) }} align="left">{row.email}</TableCell>
                        <TableCell onClick={() => { redirectToAnotherPage(row.id) }} align="left">{row.totalorders}</TableCell>
                        <TableCell onClick={() => { redirectToAnotherPage(row.id) }} align="left">{row.lastorders}</TableCell>
                      </TableRow>
                    );
                  })}
                {emptyRows > 0 && (
