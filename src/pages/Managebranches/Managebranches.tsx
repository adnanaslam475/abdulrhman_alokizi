...
import cx from "./Managebranches.module.scss";
...
const rows = [
  {
    name: "فرع الاحساء",
    reference:"B01",
    taxgroup:"15% tax group",
    created:"2020-03-03 04:17pm",
...
function EnhancedTable() {
  ...
  useEffect(() => {
    ...
    const rowsData = userBranchListItems?.map((item: any) => {
      console.log(item,"itemmmm")
      return {
        created_at: item.created_at,
        groupTex: item.groupTex,
        name: item.name,
        ref_number: item.ref_number
      };
    });
    setRows(rowsData);
    if(userBranchListItems?.length === 0 ){
                ...
                ?.map((row:any, index:number) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell onClick={() => { redirectToAnotherPage(row?.id) }} align="left">{row?.ref_number}</TableCell>
                      <TableCell onClick={() => { redirectToAnotherPage(row?.id) }} align="left">{row?.groupTex}</TableCell>
                      <TableCell onClick={() => { redirectToAnotherPage(row?.id) }} align="left">{row?.created_at}</TableCell>
                     </TableRow>
                  );
                })}
              {emptyRows > 0 && (
